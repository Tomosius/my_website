...
<script lang="ts">
	...
	const isActive = (href: `/${string}`) => {
		const target = stripSlash(resolve(href, {}));
		const current = stripSlash(pathname);
		if (target === stripSlash(resolve('/', {}))) return current === target; // Home only on '/'
	...
	<div class="mx-auto max-w-6xl px-4">
		...
		>
			<a
				...
				aria-label="Home"
			>
				<img src={asset('/brand/logo.png')} alt="pecuk.dev — Home" class="h-8 w-auto" />
			</a>

			<nav class="ml-1 hidden items-center gap-1 md:flex" aria-label="Primary navigation (desktop)">
